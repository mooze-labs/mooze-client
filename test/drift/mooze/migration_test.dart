...
void main() {
  ...
  test('migration from v1 to v2 does not corrupt data', () async {
    ...
    final oldPegsData = <v1.PegsData>[];
    final expectedNewPegsData = <v2.PegsData>[];
    ...
    await verifier.testWithDataIntegrity(
      ...
      createItems: (batch, oldDb) {
        ...
        batch.insertAll(oldDb.pegs, oldPegsData);
      },
      validateItems: (newDb) async {
        expect(expectedNewSwapsData, await newDb.select(newDb.swaps).get());
        expect(expectedNewPegsData, await newDb.select(newDb.pegs).get());
      },
    );
  });
}
