...
class BinancePriceCache {
  ...
  bool get _shouldRefresh => _lastFetch == null || 
  ...
  TaskEither<String, List<Map<String, dynamic>>> getCachedPrices(BinanceApi api) {
    if (_shouldRefresh) {
  ...
  TaskEither<String, List<List<dynamic>>> getCachedKlines(BinanceApi api, String symbol, String interval, int startTime, int endTime) {
...
class BinanceApi {
  final String baseUrl = binanceApiUrl;

  BinanceApi();

  TaskEither<String, List<Map<String, dynamic>>> fetchPrices() {
    final url = Uri.https(binanceApiUrl, "ticker/24hr", {"symbols": symbols});

    return _get(url).flatMap((response) {
      if (response.statusCode != 200) {
        return TaskEither.left("Failed to query Binance API: ${response.statusCode}");
      }
  ...
  TaskEither<String, List<List<dynamic>>> fetchKlines(String symbol, String interval, int startTime, int endTime) {
    ...
    return _get(url).flatMap((response) {
      if (response.statusCode != 200) {
        return TaskEither.left("Failed to query Binance Klines API: ${response.statusCode}");
      }

      return TaskEither.right(List<List<dynamic>>.from(jsonDecode(response.body)));
    });
  }
