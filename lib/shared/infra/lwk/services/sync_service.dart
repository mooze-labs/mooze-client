...
class SyncCompleted extends SyncState { final DateTime timestamp; SyncCompleted(this.timestamp); }
class SyncFailed extends SyncState { final String error; SyncFailed(this.error); }
...
Future<void> _performSyncInIsolate(LiquidDataSource dataSource) async => await dataSource.sync();

Stream<int> createPeriodicTicker(Duration interval) => Stream.periodic(interval, (count) => count);
...
class LwkSyncService {
  ...
  TaskEither<String, Unit> startPeriodicSync(LiquidDataSource dataSource, Duration interval) {
    return TaskEither.tryCatch(
        () async {
          ...
          _syncSubscription = syncCommands.throttle(interval).asyncMap(
                  (syncTask) async {
