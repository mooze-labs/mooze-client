...
final assetQuoteProvider = FutureProvider.autoDispose
    .family<Either<String, Option<double>>, Asset>((ref, asset) async {
      final priceService = ref.read(priceServiceProvider);

      return await priceService
          .flatMap(
            (service) =>
                service.getCoinPrice(asset, optionalCurrency: Currency.brl),
          )
          .run();
    });

final legacyAssetQuoteProvider =
    FutureProvider.autoDispose<Either<String, Option<double>>>((ref) async {
