...
final depositDataProvider = FutureProvider.autoDispose.family<Either<String, PixDeposit>, String>((ref, depositId) async {
  final controller = await ref.read(pixDepositControllerProvider.future);

  return await controller.fold((err) => left("Falha ao acessar banco de dados: $err"),
      (controller) async => await controller.getDeposit(depositId).run().then(
